...
Builder simpleData(BuilderOptions options) => new SharedPartBuilder([new SimpleDataGenerator()], 'simple_data');
...
bool isSimpleDataAnnotation(ElementAnnotation a) => a.computeConstantValue().type.name.toString() == "SimpleData";
...
String _getCustomEquality(List<ElementAnnotation> annotations) {
  ...
  final annotation = annotations.firstWhere((a) => a.computeConstantValue().type.name == "CustomEquality", orElse: () => null);
  if (annotation != null) {
...
String _generateDataType(Element element) {
  ...
  final fields = classElement.accessors
      .where((m) => m.isAbstract && m.isGetter)
      .map((f) => Field(f.name, f.type.returnType.toString().replaceAll('\$', ''), _getCustomEquality(f.metadata)));

  final fieldDeclarations = fields.map((f) => 'final ${f.type} ${f.name};');
  final constructor = 'const $className({${fields.map((f) => '@required this.' + f.name).join(',')}});';
  ...
  final equality = '@override bool operator ==(other) => ${([
  ...
  final dataClass =
      'class $className extends ${element.name} { ${fieldDeclarations.join()} $constructor $copyWith $toString $equality $hash }';
